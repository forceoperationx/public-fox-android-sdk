...
|void|static|trackDeeplinkLaunch (Intent intent)<br><br>`intent` : 使用自定义URL scheam方式启动时的Intent|用于流失唤回广告计测的方法|
|void|static|trackDeeplinkLaunch (Activity activity)<br><br>`activity` : 使用自定义URL scheam方式唤起的Activity|用于流失唤回广告计测的方法|
...
|void|**setTrackingStateListener** (TrackingStateListener listener)|首次启动计测成功时的Callback函数。重写onComplete方法。|
